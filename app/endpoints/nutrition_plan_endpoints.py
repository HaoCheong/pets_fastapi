...
@router.post("/api/v1/nutrition_plan", response_model=schemas.NutritionPlanReadNR, tags=["Nutrition Plans"])
...
@router.get("/api/v1/nutrition_plans", response_model=List[schemas.NutritionPlanReadNR], tags=["Nutrition Plans"])
...
def delete_nutrition_plan_by_id(nutrition_plan_id: str, db: Session = Depends(get_db)):
    ...
    if not db_nutrition_plan:
        raise HTTPException(
            status_code=400, detail="nutrition_plan does not exist")
